...
class CompleteTradingSystem:
    ...
    def _create_normalized_label_data(self, df):
        ...
        # 创建归一化后的数据框
        normalized_data = scaler.fit_transform(label_df[existing_columns])
        normalized_df = pd.DataFrame(normalized_data, index=label_df.index)
        normalized_df.columns = existing_columns
        
        # 将布尔列和分类列直接复制
        boolean_columns = [col for col in label_df.columns if col.startswith('SMC_') or col in ['institutional_volume', 'large_order_flow', 'CMACD_cross_up', 'CMACD_macd_above_signal']]
        for col in boolean_columns:
            if col in label_df.columns:
                normalized_df[col] = label_df[col]
        
        # 复制价格相关列（不归一化）
        price_columns = ['open', 'high', 'low', 'close', 'MA_5', 'MA_10', 'MA_20', 'MA_42']
        ...
        def determine_label(row):
            try:
                ...
                buy_conditions = [
                    ...
                    ((row['institutional_volume'] == True) and (row['close'] > row['open']), '机构资金', 5),
                    ((row['trend_strength'] > 0.5) and (row['MA_5'] > row['MA_10']), '趋势强度', 4),  # 归一化后阈值调整
                    ((RSI_LOWER_BOUND < row['RSI6'] < RSI_UPPER_BOUND), 'RSI中性偏强', 3),
                    ((row['close'] > row['MA_10']) and (row['support_distance'] < 0.3), '支撑确认', 3),  # 归一化后阈值调整
                    ((row['money_flow_volume'] > 0.5) and (row['volume_ratio'] > 0.6), '资金流入', 3),  # 归一化后阈值调整
                    ((row['large_order_flow'] == True) and (row['close'] > row['open']), '大单流入', 4),

                    # ✅ 新增共振逻辑
                    ((row['SMC_is_BOS_Low'] == True) and (row['CMACD_momentum_acceleration'] > 0.5), 'BOS+动量共振', 5),  # 归一化后阈值调整

                    # ✅ 强化CMACD信号
                    ((row['CMACD_cross_up'] == True) and (row['CMACD_macd_above_signal'] == True), 'CMACD金叉', 6),
                    ((row['CMACD_signal_strength'] >= 0.6) and (row['volume_ratio'] > 0.5), 'CMACD强信号', 5)  # 归一化后阈值调整
                ]
                ...
                sell_conditions = [
                    ...
                    # 2. 新增：价格动量衰竭信号
                    ((row['RSI6'] > 0.8) and (row['momentum_ratio'] < -0.3) and (row['volume_ratio'] < 0.6), '动量衰竭', 5),
                    
                    # 3. 新增：背离信号检测
                    ((row['RSI6'] > 0.75) and (row['CMACD_macd'] < row['CMACD_signal']) and (row['volume_ratio'] < 0.7), 'RSI-CMACD背离', 6),
                    
                    # 4. 新增：价格结构顶部信号
                    ((row['high'] > row['MA_20'] * 1.05) and (row['body_ratio'] < 0.4) and (row['volume_ratio'] < 0.8), '价格顶部', 5),
                    
                    # 5. 新增：成交量萎缩信号
                    ((row['volume_ratio'] < 0.5) and (row['close'] < row['open']) and (row['RSI6'] > 0.7), '成交量萎缩', 4),
                    
                    # 6. 新增：趋势反转确认
                    ((row['MA_5'] < row['MA_10']) and (row['momentum_ratio'] < -0.2) and (row['trend_strength'] < 0.3), '趋势反转', 4),
