...
class CompleteTradingSystem:
    ...
    def _calculate_traditional_macd(self, df, fast=12, slow=26, signal=9):
        ...
        # MACD背离检测
        df['MACD_bullish_divergence'] = (df['close'] < df['close'].shift(5)) & (df['MACD_DIF'] > df['MACD_DIF'].shift(5))
        df['MACD_bearish_divergence'] = (df['close'] > df['close'].shift(5)) & (df['MACD_DIF'] < df['MACD_DIF'].shift(5))
        ...
        df['MACD_hist_color'] = np.where(df['MACD_histogram'] > 0, 
                                        np.where(df['MACD_histogram'] > df['MACD_histogram'].shift(1), 1, 2),  # 1: 青色, 2: 蓝色
                                        np.where(df['MACD_histogram'] < df['MACD_histogram'].shift(1), 3, 4))  # 3: 红色, 4: 深红色
    ...
    def _calculate_premium_discount_zones(self, df):
        ...
        df['in_equilibrium_zone'] = (df['close'] >= df['equilibrium_zone_bottom']) & (df['close'] <= df['equilibrium_zone_top'])
    ...
    def calculate_advanced_features(self, df):
        ...
        df['trend_consistency'] = ((df['close'] > df['MA_5']) & (df['MA_5'] > df['MA_10']) & (df['MA_10'] > df['MA_20'])).astype(int)
        ...
        # 市场效率指标
        df['market_efficiency'] = abs(df['close'] - df['close'].shift(1)).rolling(20).sum() / (df['high'].rolling(20).max() - df['low'].rolling(20).min())
    ...
    def generate_smc_labels(self, df):
        ...
        def determine_label(row):
            try:
                ...
                # 买入信号条件 (集成LuxAlgo SMC特征)
                buy_conditions = [
                    # 原有条件
                    ((row['SMC_is_BOS_High'] == True) or (row['SMC_swept_prev_high'] == True), '结构突破', 4),
                    ((row['volume_ratio'] > VOLUME_MULTIPLIER) and (row['body_ratio'] > 0.6), '流动性吸收', 4),
                    ((row['institutional_volume'] == True) and (row['close'] > row['open']), '机构资金', 4),
                    ((row['trend_strength'] > 0.015) and (row['MA_5'] > row['MA_10']), '趋势强度', 3),
                    ((RSI_LOWER_BOUND < row['RSI6'] < RSI_UPPER_BOUND), 'RSI中性', 3),
                    ((row['close'] > row['MA_10']) and (row['support_distance'] < 0.05), '支撑确认', 3),
                    ((row['volatility_contraction'] == True) and (row['bb_position'] > 0.7), '波动收缩突破', 3),
                    ((row['money_flow_volume'] > 0) and (row['volume_ratio'] > 1.2), '资金流入', 3),
                    ((row['market_sentiment'] > 0) and (row['liquidity_signal'] == True), '情绪改善', 2),
                    ((row['momentum_ratio'] > 0) and (row['roc_ratio'] > 0.02), '动量确认', 2),
                    ((row['large_order_flow'] == True) and (row['close'] > row['open']), '大单流入', 4),
                    ((row['trend_consistency'] == 1) and (row['ma5_slope'] > 0), '趋势一致', 2),
                    ((row['bb_position'] > 0.8) and (row['volume_ratio'] > 1.5), '布林带突破', 2),
                    ((row['efficiency_ratio'] > 1.2) and (row['price_momentum'] > 0.01), '市场效率', 1),
                    
                    # 新增LuxAlgo SMC条件
                    ((row['SMC_internal_bullish_bos'] == True) or (row['SMC_swing_bullish_bos'] == True), 'LuxAlgo BOS', 5),
                    ((row['SMC_internal_bullish_choch'] == True) or (row['SMC_swing_bullish_choch'] == True), 'LuxAlgo CHoCH', 5),
                    ((row['SMC_internal_bullish_ob'] == True) or (row['SMC_swing_bullish_ob'] == True), 'LuxAlgo订单块', 4),
                    ((row['SMC_bullish_fvg'] == True) and (row['SMC_fvg_filled'] == False), 'LuxAlgo看涨缺口', 4),
                    ((row['equal_lows'] == True) and (row['equal_lows_count'] >= 2), 'LuxAlgo等低支撑', 3),
                    ((row['in_discount_zone'] == True) and (row['zone_strength'] >= 2), 'LuxAlgo折价区域', 3),
                    ((row['near_daily_low'] == True) or (row['near_weekly_low'] == True), 'LuxAlgo MTF支撑', 3),
                    ((row['SMC_internal_structure_strength'] >= 1) and (row['SMC_swing_structure_strength'] >= 1), 'LuxAlgo结构强度', 3),
                    ((row['SMC_fvg_strength'] > 0.5) and (row['SMC_bullish_fvg'] == True), 'LuxAlgo强缺口', 4),
                    ((row['SMC_order_block_strength'] >= 1) and (row['volume_ratio'] > 1.2), 'LuxAlgo订单块强度', 4),
                    
                    # 新增SMC结构条件
                    ((row['SMC_is_BOS_Low'] == True) and (row['SMC_BOS_Low_Value'] is not None), 'SMC BOS低点', 5),
                    ((row['SMC_is_CHoCH_Low'] == True) and (row['SMC_CHoCH_Low_Value'] is not None), 'SMC CHoCH低点', 5),
                    ((row['SMC_Strong_Low'] is not None) and (row['close'] > row['SMC_Strong_Low']), 'SMC强支撑', 4),
                    
                    # 新增Squeeze Momentum条件
                    ((row['SMI_squeeze_breakout_bullish'] == True) and (row['SMI_squeeze_momentum'] > 0), 'Squeeze看涨突破', 5),
                    ((row['SMI_momentum_reversal_bullish'] == True) and (row['SMI_momentum_strength'] > 0.5), 'Squeeze动量反转', 4),
                    ((row['SMI_squeeze_momentum'] > 0) and (row['SMI_momentum_acceleration'] > 0), 'Squeeze动量加速', 3),
                    ((row['SMI_squeeze_off'] == True) and (row['SMI_squeeze_momentum'] > 0), 'Squeeze释放看涨', 4),
                    ((row['SMI_momentum_color'] == 1) and (row['volume_ratio'] > 1.2), 'Squeeze绿色动量', 3),
                    ((row['SMI_squeeze_momentum_signal'] >= 3) and (row['volume_ratio'] > 1.1), 'Squeeze强信号', 4),
                    
                    # 新增CMACD条件
                    ((row['CMACD_cross_up'] == True) and (row['CMACD_macd_above_signal'] == True), 'CMACD金叉', 5),
                    ((row['CMACD_cross_zero_up'] == True) and (row['CMACD_macd_momentum'] > 0), 'CMACD零线上穿', 4),
                    ((row['CMACD_bullish_divergence'] == True) and (row['CMACD_macd_above_signal'] == True), 'CMACD看涨背离', 4),
                    ((row['CMACD_trend_consistency'] == 1) and (row['CMACD_histogram'] > 0), 'CMACD趋势一致', 3),
                    ((row['CMACD_mtf_consistency'] == 1) and (row['CMACD_macd_above_signal'] == True), 'CMACD多时间框架一致', 4),
                    ((row['CMACD_macd_above_signal'] == True) and (row['CMACD_hist_A_up'] == True), 'CMACD强势上涨', 3),
                    ((row['CMACD_signal_strength'] >= 3) and (row['volume_ratio'] > 1.1), 'CMACD强信号', 4),
                    ((row['CMACD_relative_strength'] > 1.5) and (row['CMACD_macd_above_signal'] == True), 'CMACD相对强度', 3),
                    ((row['CMACD_momentum_acceleration'] > 0) and (row['CMACD_macd_above_signal'] == True), 'CMACD动量加速', 3),
                    ((row['CMACD_hist_A_up'] == True) and (row['volume_ratio'] > 1.2), 'CMACD直方图上涨', 3)
                ]
                
                # 卖出信号条件 (集成LuxAlgo SMC特征)
                sell_conditions = [
                    # 原有条件
                    ((row['SMC_is_CHoCH_High'] == True) or (row['SMC_swept_prev_low'] == True), '结构破坏', 4),
                    ((row['RSI6'] > 75) and (row['SMC_bearish_ob'] == True), '超买背离', 4),
                    ((row['MA_5'] < row['MA_10']) and (row['momentum_ratio'] < -0.01), '趋势反转', 4),
                    ((row['volume_ratio'] < 0.8) and (row['close'] < row['open']), '流动性耗尽', 3),
                    ((row['drawdown_ratio'] > 0.05), '大幅回撤', 4),
                    ((row['close'] < row['MA_20']) and (row['support_distance'] > 0.1), '支撑破位', 3),
                    ((row['money_flow_volume'] < 0) and (row['volume_ratio'] > 1.5), '资金流出', 3),
                    ((row['market_sentiment'] < -0.5) and (row['liquidity_ratio'] < row['liquidity_ma'] * 0.8), '情绪恶化', 2),
                    ((row['rsi_oversold'] == 1) and (row['close'] < row['MA_20']), '超卖确认', 2),
                    ((row['momentum_acceleration'] < -0.02) and (row['volume_ratio'] < 0.9), '动量衰竭', 2),
                    ((row['bb_position'] < 0.2) and (row['volume_ratio'] > 1.3), '布林带下轨', 2),
                    ((row['efficiency_ratio'] < 0.8) and (row['price_momentum'] < -0.01), '效率下降', 1),
                    
                    # 新增LuxAlgo SMC条件
                    ((row['SMC_internal_bearish_bos'] == True) or (row['SMC_swing_bearish_bos'] == True), 'LuxAlgo BOS', 5),
                    ((row['SMC_internal_bearish_choch'] == True) or (row['SMC_swing_bearish_choch'] == True), 'LuxAlgo CHoCH', 5),
                    ((row['SMC_internal_bearish_ob'] == True) or (row['SMC_swing_bearish_ob'] == True), 'LuxAlgo订单块', 4),
                    ((row['SMC_bearish_fvg'] == True) and (row['SMC_fvg_filled'] == False), 'LuxAlgo看跌缺口', 4),
                    ((row['equal_highs'] == True) and (row['equal_highs_count'] >= 2), 'LuxAlgo等高阻力', 3),
                    ((row['in_premium_zone'] == True) and (row['zone_strength'] >= 1), 'LuxAlgo溢价区域', 3),
                    ((row['near_daily_high'] == True) or (row['near_weekly_high'] == True), 'LuxAlgo MTF阻力', 3),
                    ((row['SMC_internal_structure_strength'] >= 1) and (row['SMC_swing_structure_strength'] >= 1), 'LuxAlgo结构强度', 3),
                    ((row['SMC_fvg_strength'] > 0.5) and (row['SMC_bearish_fvg'] == True), 'LuxAlgo强缺口', 4),
                    ((row['SMC_order_block_strength'] >= 1) and (row['volume_ratio'] > 1.2), 'LuxAlgo订单块强度', 4),
                    
                    # 新增SMC结构条件
                    ((row['SMC_is_BOS_High'] == True) and (row['SMC_BOS_High_Value'] is not None), 'SMC BOS高点', 5),
                    ((row['SMC_is_CHoCH_High'] == True) and (row['SMC_CHoCH_High_Value'] is not None), 'SMC CHoCH高点', 5),
                    ((row['SMC_Weak_High'] is not None) and (row['close'] < row['SMC_Weak_High']), 'SMC弱阻力', 4),
                    
                    # 新增Squeeze Momentum条件
                    ((row['SMI_squeeze_breakout_bearish'] == True) and (row['SMI_squeeze_momentum'] < 0), 'Squeeze看跌突破', 5),
                    ((row['SMI_momentum_reversal_bearish'] == True) and (row['SMI_momentum_strength'] > 0.5), 'Squeeze动量反转', 4),
                    ((row['SMI_squeeze_momentum'] < 0) and (row['SMI_momentum_acceleration'] < 0), 'Squeeze动量加速', 3),
                    ((row['SMI_squeeze_off'] == True) and (row['SMI_squeeze_momentum'] < 0), 'Squeeze释放看跌', 4),
                    ((row['SMI_momentum_color'] == 3) and (row['volume_ratio'] > 1.2), 'Squeeze红色动量', 3),
                    ((row['SMI_squeeze_momentum_signal'] <= -3) and (row['volume_ratio'] > 1.1), 'Squeeze强信号', 4),
                    
                    # 新增CMACD条件
                    ((row['CMACD_cross_down'] == True) and (row['CMACD_macd_below_signal'] == True), 'CMACD死叉', 5),
                    ((row['CMACD_cross_zero_down'] == True) and (row['CMACD_macd_momentum'] < 0), 'CMACD零线下穿', 4),
                    ((row['CMACD_bearish_divergence'] == True) and (row['CMACD_macd_below_signal'] == True), 'CMACD看跌背离', 4),
                    ((row['CMACD_trend_consistency'] == 0) and (row['CMACD_histogram'] < 0), 'CMACD趋势破坏', 3),
                    ((row['CMACD_mtf_consistency'] == 0) and (row['CMACD_macd_below_signal'] == True), 'CMACD多时间框架破坏', 4),
                    ((row['CMACD_macd_below_signal'] == True) and (row['CMACD_hist_B_down'] == True), 'CMACD强势下跌', 3),
                    ((row['CMACD_signal_strength'] <= -3) and (row['volume_ratio'] > 1.1), 'CMACD强信号', 4),
                    ((row['CMACD_relative_strength'] < -1.5) and (row['CMACD_macd_below_signal'] == True), 'CMACD相对强度', 3),
                    ((row['CMACD_momentum_acceleration'] < 0) and (row['CMACD_macd_below_signal'] == True), 'CMACD动量加速', 3),
                    ((row['CMACD_hist_B_down'] == True) and (row['volume_ratio'] > 1.2), 'CMACD直方图下跌', 3)
                ]
                
                # 计算得分
                buy_score = sum(weight for condition, reason, weight in buy_conditions if condition)
                sell_score = sum(weight for condition, reason, weight in sell_conditions if condition)
                
                # 风险评估 (集成LuxAlgo特征)
                risk_factors = [
                    row['drawdown_ratio'] > 0.03,
                    row['volatility_ratio'] > 0.05,
                    row['price_volatility'] > 0.04,
                    row['volume_volatility'] > 2.0,
                                    # 新增LuxAlgo风险因子
                row['SMC_fvg_strength'] > 2.0,  # 过大缺口风险
                row['mtf_strength'] >= 3,   # 多时间框架阻力风险
                row['zone_strength'] >= 3,  # 强区域风险
                row['SMC_order_block_strength'] >= 3,  # 强订单块风险
                
                # 新增Squeeze Momentum风险因子
                row['SMI_squeeze_momentum_signal'] > 5,  # 过强Squeeze信号风险
                row['SMI_squeeze_momentum_signal'] < -5,  # 过强Squeeze信号风险
                row['SMI_momentum_strength'] > 2.0,  # 过强动量风险
                row['SMI_squeeze_on'] == True,  # 挤压状态风险
                
                # 新增CMACD风险因子
                row['CMACD_signal_strength'] > 8,  # 过强CMACD信号风险
                row['CMACD_signal_strength'] < -8,  # 过强CMACD信号风险
                row['CMACD_relative_strength'] > 3.0,  # 过强相对强度风险
                row['CMACD_relative_strength'] < -3.0,  # 过强相对强度风险
                row['CMACD_volatility'] > 0.05,  # 高CMACD波动率风险
                row['CMACD_momentum_acceleration'] > 0.02,  # 过强动量加速风险
                row['CMACD_momentum_acceleration'] < -0.02  # 过强动量加速风险
                ]
                risk_score = sum(risk_factors)
                
                # 决策逻辑
                min_buy_score = 10  # 提高阈值
                min_sell_score = 9
                
                if buy_score >= min_buy_score and buy_score > sell_score:
                    confidence = min(buy_score / 25, 1.0)  # 调整分母
                    ...
                    confidence = min(sell_score / 25, 1.0)
                    risk_level = '低' if risk_score <= 2 else '中' if risk_score <= 4 else '高'
        ...
        # 应用标签生成
        results = df.apply(determine_label, axis=1)
        
        # 确保结果列表长度与DataFrame长度一致
        results_list = results.tolist()
        if len(results_list) != len(df):
            print(f"警告: 结果长度 {len(results_list)} 与数据长度 {len(df)} 不匹配")
            # 填充缺失的结果
            while len(results_list) < len(df):
                results_list.append(('0', 0.5, '填充信号', '低'))
            results_list = results_list[:len(df)]
        
        # 创建结果DataFrame
        results_df = pd.DataFrame(results_list, columns=['label', 'confidence', 'signal_reason', 'risk_level'])
        ...
        df['risk_score'] = df['drawdown_ratio'] * df['volatility_ratio'] * 100
    ...
    def process_complete_system(self, symbol, interval, start_date, end_date=None):
        ...
        # 10. 保存结果
        output_file = f"complete_dataset_{symbol}_{interval}_squeeze_luxalgo_advanced.csv"
    ...
    def _print_statistics(self, df, output_file):
        ...
        if len(signal_quality) > 0:
            ...
            # LuxAlgo特征在信号中的分布
            luxalgo_features = {
                'LuxAlgo BOS': signal_quality['SMC_internal_bullish_bos'].sum() + signal_quality['SMC_swing_bullish_bos'].sum(),
                'LuxAlgo CHoCH': signal_quality['SMC_internal_bullish_choch'].sum() + signal_quality['SMC_swing_bullish_choch'].sum(),
                'LuxAlgo订单块': signal_quality['SMC_internal_bullish_ob'].sum() + signal_quality['SMC_swing_bullish_ob'].sum(),
    ...
    def plot_macd_chart(self, df, symbol="BTCUSDT", save_path=None):
        ...
        # 标记背离点
        bullish_div_points = df[df['MACD_bullish_divergence'] == True]
        bearish_div_points = df[df['MACD_bearish_divergence'] == True]
        
        if len(bullish_div_points) > 0:
            ax1.scatter(bullish_div_points['open_time'], bullish_div_points['close'], 
                       color='lime', marker='o', s=80, label='看涨背离', zorder=5)
        
        if len(bearish_div_points) > 0:
            ax1.scatter(bearish_div_points['open_time'], bearish_div_points['close'], 
                       color='magenta', marker='o', s=80, label='看跌背离', zorder=5)
